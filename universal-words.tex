...
Neither the statement nor the proof below uses the standing assumption
\eqref{eq:standing}: the argument depends on $(r,s)$ only through $|s|$, $|rs|$ and
the primes dividing $rs$, so Proposition~\ref{prop:good} holds as stated for all
nonzero integers $r,s$, of either parity and either sign, including
$m(r,s)=1$ (where the count $\tilde\omega$ is $0$ and \eqref{eq:omegabound} is
immediate).
